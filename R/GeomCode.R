#' Loading the data using instructions provided
#'
...
#' @example
#' \dontrun{
#'   load_geom_data("data/ebtrk_atlc_1988_2015.txt")
#' }
#'
#' @export

load_geom_data <- function(filename) {
...
#' Using the hurricane data and tidying it up
#'
#' @importFrom stringr str_c str_to_title
#' @importFrom dplyr mutate_ select_
#' @importFrom tidyr gather spread
#'
#' @param data input hurricane data to tidy up
#' @example
#' \dontrun{
#'   tidy_geom_data(ext_tracks)
#' }
#'
#' @export
tidy_geom_data <- function(data) {
  data %>%
    ...
    mutate_(wind_speed = ~str_extract(variable, "(34|50|64)"), variable = ~str_extract(variable, "(ne|nw|se|sw)")) %>%
...
#' Filter the data by hurricane name and certain period or date
#'
#' @importFrom dplyr filter_
#'
#' @param data hurricane data sent as an input for selection
#' @param hurr_name name of hurricane to be filtered
#' @param timing_obs time to filter the hurricane data
#'
#' @example
#' \dontrun{
#'   filter_geom_data(ext_tracks, "Ike-2008","2008-09-05 00:00:00")
#' }
#' @export
filter_geom_data <- function(data, hurr_name, timing_obs) {
  data <- filter_(data, ~storm_id == hurr_name & date == timing_obs)
...
#'                level of the plot. You must supply mapping if
#'                there is no plot mapping.
#' @param data The data to be displayed in this layer. There are three
#'                options: If NULL, the default, the data is inherited
#'                from the plot data as specified in the call to ggplot.
#'                A data.frame, or other object, will override the plot
#'                data. All objects will be fortified to produce a data
#'                frame. See fortify for which variables will be created.
#'                A function will be called with a single argument, the
#'                plot data. The return value must be a data.frame., and
#'                will be used as the layer data.
#' @param stat The statistical transformation to use on the data for this
#'                layer, as a string.
#' @param position Position adjustment, either as a string, or the result
#'                of a call to a position adjustment function.
...
#'                than combining with them. This is most useful for helper
#'                functions that define both data and aesthetics and
...
#' Function to construct a new class for the geom_hurricane, this function
#' will create a wind radii for a given storm obseravtion. It is to be used
#' with maps construct with get_map
#'
...
#'                    libraysupplied in character vector
...
geom_hurricane_proto <-
  ggplot2::ggproto("geom_hurricane_proto", Geom,
                   ...
                   draw_group = function(data, panel_scales, coord) {
                     ...
                     for (i in 1:nrow(data)) {
                       ...
                       df_points <- dplyr::bind_rows(df_nw, df_ne, df_se, df_sw)
                     ...
                     grid::polygonGrob(
                       x= coords_df$x,
                       y = coords_df$y,
                       gp = grid::gpar(col = coords_df$colour, fill = coords_df$fill,
